...
//  StringsTest
//
//  Created by Горявин Владимир on 21.01.2019.
...
/*----- Level 1:
  // -- 2:
    // Separating a string into two equal parts
    NSUInteger halfLength = text.length / 2;
    
    // Create a range from the first character to string's half length
    NSRange rangeLeft = NSMakeRange(0, halfLength);
    
    // Left part of string
    NSString* textLeft = [text substringWithRange: rangeLeft];
    
    NSLog(@"LEFT HALF: %@", textLeft);
    
    // Location of the first character on the right side of the string
    NSInteger locationRight = rangeLeft.location + rangeLeft.length;
    
    // Create a range from the "locationRight" to string's half length
    NSRange rangeRight = NSMakeRange(locationRight, halfLength);
    
    // Right part of string
    NSString* textRight = [text substringWithRange: rangeRight];
    
    NSLog(@"RIGHT HALF: %@", textRight);

  // -- 3:
    NSArray* arrayString = [text componentsSeparatedByString:@". "];

    for (int i  = 0; i < arrayString.count; i ++) {
        
        if(i < arrayString.count - 1) {
        
            NSLog(@"%@.", (NSString*) arrayString[i]);
...
/*----- Level 2:
  // -- 4:
    // Source array. Elements array are sentences
    NSArray* arrayString = [text componentsSeparatedByString:@". "];
    
    // Output array. Elements array are sentences containing the word "NSString"
    NSMutableArray* arrayWithNSString = [[NSMutableArray alloc] init];
    
    for (NSString* string in arrayString) {
        
        // Search "NSString" in string
        NSRange range = [string rangeOfString: @"NSString"];
        
        if (range.location != NSNotFound) { // If "NSString" found
            
            // save this string in the array "arrayWithNSString"
            [arrayWithNSString addObject: string];
        }
    }
...
/*----- Level 3:
  // -- 5:
    NSArray* arrayWord = [[NSArray alloc] init];
    
    NSMutableArray* arrayWithChangedWordOrder;

    NSMutableArray* arrayOfStrings = [[NSMutableArray alloc] init];
    
    NSString* stringWithChangedWordOrder;
    
    for (NSString* string in arrayWithNSString) {
        
        // Split string into words
        arrayWord = [string componentsSeparatedByString:@" "];
        
        NSString *firstWord, *lastWord;
        
        NSInteger indexLastWord = arrayWord.count - 1;
        
        // Save first word in "arrayWord"
        firstWord = [arrayWord objectAtIndex: 0];
        
        // Save last Word in "arrayWord"
        lastWord = [arrayWord objectAtIndex: indexLastWord];
        
        // Copy "arrayWord" in "arrayWithChangedWordOrder"
        arrayWithChangedWordOrder = [[NSMutableArray alloc] initWithArray: arrayWord];
        
        // Сhange the order of the words in "arrayWithChangedWordOrder":
            // the first word will be the last word
        [arrayWithChangedWordOrder replaceObjectAtIndex: 0 withObject: lastWord];
 
            //the last word will be the first word
        [arrayWithChangedWordOrder replaceObjectAtIndex: indexLastWord withObject: firstWord];
        
        // Сombine the words in a string
        stringWithChangedWordOrder = [arrayWithChangedWordOrder componentsJoinedByString:@" "];

        // Add string to array "arrayOfStrings"
        [arrayOfStrings insertObject: stringWithChangedWordOrder atIndex: 0];
    }
...
/*----- Level 4: -----*/
  // -- 6:
    ...
    NSMutableArray* arrayChangedWord = [[NSMutableArray alloc] init];
    
    NSString* word;
    
    unichar letterFirst, letterLast;
    
    // Break the string into words
    arrayWord = [text componentsSeparatedByString:@" "];
    ...
    for (int i = 0; i < arrayWord.count; i ++) {
        ...
        // Change the first letter in the word to lowercase
        letterFirst = tolower([word characterAtIndex:0]);
        
        // Change the last letter in the word to uppercase
        letterLast = toupper([word characterAtIndex: word.length - 1]);
        
        // The range for the first letter
        NSRange range = NSMakeRange(0, 1);
        
        // The new word equal to the original word with the changed first letter
        NSString* newWord = [word stringByReplacingCharactersInRange:range withString: [NSString stringWithFormat:@"%c", letterFirst]];
        
        // The range for the last letter
        range = NSMakeRange(word.length - 1, 1);
        
        // The new word equal to the original word with the changed first and last letter
        newWord = [newWord stringByReplacingCharactersInRange:range withString: [NSString stringWithFormat:@"%c", letterLast]];
        
        // Add new word in array
        [arrayChangedWord addObject: newWord];
    }
    
    //The string containing changed words separated by a space
    text = [arrayChangedWord componentsJoinedByString:@" "];
