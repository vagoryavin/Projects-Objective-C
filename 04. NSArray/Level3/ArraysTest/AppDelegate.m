...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    NSArray* arrayPeopleAndAnimals = [[NSArray alloc] initWithObjects: human, animal, cyclist, dog, runner, cat, swimmer, bird, child, nil];
    
    for (int i = 0; i < arrayPeopleAndAnimals.count; i++){
        
        Human* object = arrayPeopleAndAnimals[i];
        
        if([object isKindOfClass: [Human class]]){
            
            NSLog(@"----- HUMAN:");
            NSLog(@"name: %@; height: %f; weight: %f; gender: %@", object.name, object.height, object.weight, object.gender == genderMale ? @"Male" : @"Female");
            
            if([object isKindOfClass: [Child class]]){
            ...
            Animal* object = arrayPeopleAndAnimals[i];
            
            NSLog(@"----- ANIMAL:");
            NSLog(@"nickname: %@; age: %ld", object.nickname, object.age);
